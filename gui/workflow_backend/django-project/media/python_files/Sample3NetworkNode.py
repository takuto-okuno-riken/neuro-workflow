...
class Sample3SonataNetworkNode(Node):
    """ ** Sampke ** Simulation of a NEST network built from SONATA."""
    
    NODE_DEFINITION = NodeDefinitionSchema(
        type='sample_simulation_node',
        description='Simulates a NEST network built from SONATA (Sample)',
        parameters={
            'sample_simulation_time': ParameterDefinition(
                default_value=1000.0,
                description='Simulation time in milliseconds (Sample)',
                constraints={'min': 0.0}
            ),
            'sample_record_from_population':ParameterDefinition(
                default_value='internal',
                description='Any population of interest (Sample)'
            ),
            'sample_record_n_neurons':ParameterDefinition(
                default_value='100',
                description='Number of neurons of record_from_population from where spikes are recorded (Sample)',
                constraints={'min':1}
            )
        },
        inputs={
            'sample_sonata_net': PortDefinition(
                type=PortType.OBJECT,
                description='SONATA network object (Sample)'
            ),
            'sample_node_collections': PortDefinition(
                type=PortType.DICT,
                description='NEST node collections (Sample)'
            )
        },
        outputs={
            'sample_simulation_completed': PortDefinition(
                type=PortType.BOOL,
                description='Whether simulation completed successfully (Sample)'
            ),
            'sample spike_recorder': PortDefinition(
                type=PortType.OBJECT,
                description='NEST spike recorder device (Sample)'
            )
        },
        methods={
            'sample_initialize_recordables': MethodDefinition(
                description='Setup spike recorders (sample)',
                inputs=['sample_node_collections'],
                outputs=['spike_recorder']
            ),
            'sample_simulate_sonata': MethodDefinition(
                description='Run simulation (sample)',
                inputs=['sample_sonata_net', 'sample_node_collections', 'sample_spike_recorder'],
                outputs=['sample_simulation_completed']
            )
        }
    )
